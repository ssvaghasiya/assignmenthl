...
import com.assignmenthyperlink.BuildConfig
...
//        mIntent.putExtra(Intent.EXTRA_SUBJECT, "");
    ...
    fun sendCrashReport(stackStrace: String, mContext: Activity) {
        if (Debug.DEBUG_EXCEPTION) {
            ...
            /*To send an email you need to specify mailto: as URI using setData() method
        and data type will be to text/plain using setType() method*/
            mIntent.data = Uri.parse("mailto:1234@gmail.com")
            mIntent.type = "text/plain"
            // put recipient email in intent
            /* recipient is put as array because you may wanna send email to multiple emails
           so enter comma(,) separated emails, it will be stored in array*/
            ...
            mIntent.putExtra(Intent.EXTRA_EMAIL, arrayOf("1234@gmail.com"))
    ...
    fun formatNumber(yourStringPhone: String, countryCode: String): String? {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return PhoneNumberUtils.formatNumber(yourStringPhone, countryCode);
        } else {
    ...
    fun getTimeFromDate(date: Date, format: String): String {
        val localDateFormat = SimpleDateFormat(format)
        val time = localDateFormat.format(date)
        return time
    }

    fun getCurrentTimePlusHour(hour: Int): Date {
    ...
    fun updateSearchData(context: Context, recentSearch: ArrayList<String>) {
        Utils.setPref(context, Constant.PREF_RECENT_SEARCH, Gson().toJson(recentSearch))
    }

    fun getSearchData(context: Context): ArrayList<String>? {
        val recentSearch = Utils.getPref(context, Constant.PREF_RECENT_SEARCH, "")
        if (recentSearch.isNullOrEmpty().not()) {
            return Gson().fromJson<ArrayList<String>>(
                recentSearch!!, object : TypeToken<ArrayList<String>>() {}.type
            )
        }
        return null
    }
